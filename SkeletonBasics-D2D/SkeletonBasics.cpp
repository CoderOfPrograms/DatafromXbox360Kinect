...
CSkeletonBasics::CSkeletonBasics() :
    ...
    m_pNuiSensor(NULL)
{
    pWorkbook = workbook_new("output_file.xlsx");

    // Create a new worksheet within the workbook
    pWorksheet = workbook_add_worksheet(pWorkbook, NULL);
    worksheet_write_string(pWorksheet, 0, 0, "Timestamp", NULL);
    // Add headers for other columns as needed.
    // For example:
    worksheet_write_string(pWorksheet, 0, 1, "Head 1 X", NULL);
    worksheet_write_string(pWorksheet, 0, 2, "Head 1 Y", NULL);
    worksheet_write_string(pWorksheet, 0, 3, "Head 1 Z", NULL);
    worksheet_write_string(pWorksheet, 0, 4, "Neck 1 X", NULL);
    worksheet_write_string(pWorksheet, 0, 5, "Neck 1 Y", NULL);
    worksheet_write_string(pWorksheet, 0, 6, "Neck 1 Z", NULL);

    // Repeat for other joints and position data.
...
void CSkeletonBasics::ProcessSkeleton()
{
    ...
    for (int i = 0 ; i < NUI_SKELETON_COUNT; ++i)
    {
        ...
        {
            //Writes to excel
            WriteSkeletonDataToExcel(skeletonFrame.SkeletonData[i]);
...
std::string GetTimestamp()
{
    ...
    // Convert it to a tm struct for formatting
    std::tm timeInfo;
    localtime_s(&timeInfo, &now);

    // Format the timestamp as a string (e.g., "YYYY-MM-DD HH:MM:SS")
    char buffer[20]; // Buffer to store the timestamp
    strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", &timeInfo);

    return buffer;
}

// Modify the WriteSkeletonDataToExcel function
void CSkeletonBasics::WriteSkeletonDataToExcel(const NUI_SKELETON_DATA& skel)
{
    // Get the current timestamp
    std::string timestamp = GetTimestamp();

    // Get the current row for writing data (assuming you have a member variable to track the row)
    static int currentRow = 1; // Start from row 1 to skip headers

    // Write timestamp in the first column
    worksheet_write_string(pWorksheet, currentRow, 0, timestamp.c_str(), NULL);

    // Iterate through the skeleton joints and write their information to the Excel file
    for (int jointIndex = 0; jointIndex < NUI_SKELETON_POSITION_COUNT; ++jointIndex)
    {
        // Write joint position data to the Excel file
        const Vector4& jointPosition = skel.SkeletonPositions[jointIndex];
        int columnOffset = jointIndex * 3 + 1; // Each joint occupies 3 columns (X, Y, Z)

        // Write X, Y, and Z positions for the joint
        worksheet_write_number(pWorksheet, currentRow, columnOffset, jointPosition.x, NULL);
        worksheet_write_number(pWorksheet, currentRow, columnOffset + 1, jointPosition.y, NULL);
        worksheet_write_number(pWorksheet, currentRow, columnOffset + 2, jointPosition.z, NULL);
    }
